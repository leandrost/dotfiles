...
class DBusService(Borg, dbus.service.Object):
    ...
    #leandrost#4: Add dbus methods to change profile
    @dbus.service.method(BUS_NAME)
    def get_profile(self, uuid):
        """Return current terminal profile"""
        terminal = self.terminator.find_terminal_by_uuid(uuid)
        return terminal.get_profile()

    @dbus.service.method(BUS_NAME)
    def set_profile(self, uuid, profile):
        """Set current termianal profile"""
        terminal = self.terminator.find_terminal_by_uuid(uuid)
        return terminal.set_profile(None, profile)
    # leandrost#4
...
# leandrost#3: Add commands to change profiles
@with_proxy
def get_profile(session, uuid):
    """Call the dbus methods to return current terminal profile"""
    print session.get_profile(uuid)

@with_proxy
def set_profile(session, uuid, profile):
    """Call the dbus methods to set current terminal profile"""
    print session.set_profile(uuid, profile)
# leandrost#3
