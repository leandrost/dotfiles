...
class Terminal(gtk.VBox):
    ...
    def reconfigure(self, _widget=None):
        ...
        #if self.config['use_inactive_bgcolor_offset']:
        #  bgfactor = -12 #self.config['inactive_bgcolor_offset']
        #  self.bgcolor_inactive = self.bgcolor.copy()
        #  for bit in ['red', 'green', 'blue']:
        #      setattr(self.bgcolor_inactive, bit,
        #              getattr(self.bgcolor_inactive, bit) * bgfactor)
        #else:
        #  self.bgcolor_inactive = gtk.gdk.color_parse(self.config['inactive_bgcolor'])
        self.bgcolor_inactive = gtk.gdk.color_parse(self.config['inactive_bgcolor'])
    ...
    def on_vte_focus_out(self, _widget, _event):
        ...
        self.vte.set_colors(self.fgcolor_inactive, self.bgcolor_inactive, self.palette_inactive)
        self.set_cursor_color()
        self.emit('focus-out')
