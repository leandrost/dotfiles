			...
			this._command("SEARCH UNSEEN",true,Lang.bind(this,function(oImap,resp) {
					...
					if (xmatches!=null)
					{
						...
						this._command("FETCH "+xmatches[0].replace(/ /g,",")+" (FLAGS BODY.PEEK[HEADER.FIELDS (DATE FROM SUBJECT)] X-GM-MSGID)",false, Lang.bind(this,function(oImap,presp)
                          ...
                          var ACCOUNTS = { 'leandro.s.thimoteo@gmail.com': 0, 'leandro.thimoteo@myfreecomm.com': 1 }; 
                          var accountId = ACCOUNTS[this._conn._oAccount.get_account().identity];
                          var messageId = ba.toString(16);
                          m.set("link",'https://mail.google.com/mail/u/' + accountId + '/#inbox/' + messageId);
												}
												catch (err) {
