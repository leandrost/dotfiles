...
GmailFeed.prototype= {
  ...
  customMessageLink: function (link) {
    try {
      var ACCOUNTS = { 'leandro.s.thimoteo@gmail.com': 0, 'leandro.thimoteo@myfreecomm.com.br': 1 }; 
      var accountId = ACCOUNTS[this._conn._oAccount.get_account().identity];
      var re = /message_id=(.{16})/;
      var messageId = link.match(re)[1];
      if (_DEBUG) global.log('message_id: ' + messageId);
      link = 'https://mail.google.com/mail/u/' + accountId + '/#inbox/' + messageId;
    }catch(err){
      if (_DEBUG) global.log('customMessageLinkError: ' + err);
    }
    return link;
  },
	scanInbox: function (callback) {
		try {
			...
			sess.queue_message(msg,Lang.bind(this,function(sess,msg,callback){
					...
					if (msg.status_code==200){
						try {
							...
							while(typeof(oxml.entry[i]) !='undefined') {
								...
								em.link=this.customMessageLink(oxml.entry[i].link.@href.toString());
                  ...
                  cm.link=this.customMessageLink(oxml.entry[i].link.@href);
